...
`cpBound` 是 `map[int]int`，记录每个 turn 开始时消息日志的长度（切分位置）。用于 `/fork` 判定。

### cpBound 生命周期

| 时机 | 操作 | 代码 |
|------|------|------|
| **建立** | `beginCheckpoint()` → `cpBound[turn] = msgIndex` | `controller.go:396` |
| **恢复** | 从 checkpoint 文件重建 | `controller.go:379-381` |
| **Summarize** | **全部清空** → `cpBound = map[int]int{}` | `controller.go:1771` |
| **Rewind** | 删除 >= turn 的条目 | `controller.go:1501-1504` |

### fork 按钮禁用原因

```
checkpoint.canConversation == false ← cpBound[turn] 不存在
```

**常见导致禁用的操作**: Summarize（全部清空）、Conversation Rewind（部分删除）、会话恢复（旧 turn 无记录）。
