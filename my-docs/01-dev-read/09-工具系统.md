...
## 八、bash.ReadOnly() 设计理由

`bash` 工具的 `ReadOnly()` 返回 `false`，因为 shell 命令的副作用无法从参数静态判定——即使是 `git status` 这样的只读命令。这意味着：

- **Planner 无法用 bash** — `PlannerToolRegistry` 只保留 ReadOnly 工具
- **并行执行受限** — bash 永远走串行批
- **需用户批准**（非 YOLO 模式下）

无法"只允许 git status 而不允许 rm -rf"——工具过滤是工具级别的，不是命令级别。
