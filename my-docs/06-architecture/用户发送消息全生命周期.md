...
sequenceDiagram
    ...
    Note over User,Event: Shell 命令（如 "!ls"）
...
handleSend(trimmed, submitText)
  │
  ├─ trimmed.startsWith("!") → runShell(cmd)       ← App.tsx:1440
  │   └─ 参见 A 分支（Shell 命令）
  │
  ├─ /model /memory /clear /goal /theme 等前置命令 ← App.tsx:1449-1504
  │   └─ 直接处理返回，不走 turn
  │
