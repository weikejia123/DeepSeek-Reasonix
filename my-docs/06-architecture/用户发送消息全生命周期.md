...
| 层 | 位置 | 拦截的命令 | 行为 |
|:-:|------|-----------|------|
| **1** | 前端 `handleSend`（App.tsx:1436） | `!`, `/model`, `/memory`, `/clear`, `/goal`, `/theme` | `!` 和 `/goal` 处理后仍发往后端；其余纯前端处理 |
| **2** | 后端 `submit()`（controller.go:773） | `#`, `/remember`, `/goal`, `/compact`, `/new`, `/clear`, `/tree`, `/branch`, `/switch`, `/rewind`, `/mcp__`, 管理命令, 自定义命令, Skill | 后端直接处理，不走 agent turn |
| **default** | `runRefTurn`（controller.go:907） | 以上均不匹配 | `runGuarded` → `agent.Run` → 正常 turn |

> `/goal` 特殊：前端执行 `applyGoal(arg)` 后**仍通过 `commitThenSend` 发往后端**，后端 `submit()` 再次匹配 `/goal` 后启动 Goal 模式循环。  
> `!` 特殊：前端执行 `dispatchTo("user")` + `RunShellForTab`，后端 `submit()` 中有兜底的 `!` 匹配。  
> 前端已拦截的命令（`/model`、`/memory` 等）不会被发往后端，后端 `submit()` 不会收到它们。
